...
case $packages in
	y)
		...
		sleep 3;
		$doas xbps-install -Syu git xorg-minimal font-hack-ttf libXinerama-deve libX11-devel libXft-devel harfbuzz-devel bspwm sxhkd polybar vim base-devel xrandr setxkbmap xwallpaper xset zsh dbus elogind;;
	n)
		clear;
...
case $compile in
	y)
		...
		mkdir -p $sourcesdir
		git clone https://github.com/lukesmithxyz/st $sourcesdir/;
		git clone https://git.suckless.org/dmenu $sourcesdir/;
...
read -p "do you want me to start elind and dbus? [runit] (Y/n)" services
case $services in
	y)
